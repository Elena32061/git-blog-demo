...
lazy val chipyard = conditionalDependsOn(project in file("generators/chipyard"))
  .dependsOn(boom, hwacha, sifive_blocks, sifive_cache, utilities, iocell,
    ...
    gemmini, icenet, tracegen, ariane, nvdla,LCMRoCCAccel)
  .settings(commonSettings)


//lazy val chipyard = (project in file(".")).settings(commonSettings).dependsOn(testchipip, LCMRoCCAccel)

lazy val LCMRoCCAccel = (project in file("generators/LCMRoCCAccel"))
